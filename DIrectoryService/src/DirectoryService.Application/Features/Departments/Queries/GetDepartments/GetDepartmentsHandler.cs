    ...
    : IQueryHandler<GetDepartmentsDto, GetDepartmentsQuery>
{
    public async Task<Result<GetDepartmentsDto, Errors>> Handle(GetDepartmentsQuery query, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(query.Request.Search)) 
            queryable = queryable.Where(d => EF.Functions.Like(d.Name.Value, $"%{query.Request.Search}%"));

        queryable = query.Request.SortBy switch
        {
            //sort
            SortBy.Name => query.Request.SortDir == SortDirection.Asc
                ? queryable.OrderBy(d => d.Name)
                : queryable.OrderByDescending(d => d.Name),
            SortBy.CreatedAt => query.Request.SortDir == SortDirection.Asc
